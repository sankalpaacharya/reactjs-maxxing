...
export function CodeBlockClient({
  ...
  const deviconClass =
    deviconMap[normalizedLanguage.toLowerCase()] ||
    deviconMap[normalizedLanguage];
  ...
  const handleDownload = () => {
    const extension = getFileExtension(normalizedLanguage);
  ...
  return (
    <div
      className={cn(
        "not-prose relative my-4 rounded-lg border overflow-hidden",
            ...
            <code className="text-sm text-zinc-800 dark:text-zinc-100">
              {showLineNumbers ? (
                <div className="flex">
                  ...
                  <div className="flex-1">{codeText}</div>
                </div>
              ) : (
                codeText
              )}
