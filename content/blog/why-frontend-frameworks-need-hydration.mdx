...
React maintains something called a fiber tree internally. Think of it as React's own in-memory model of your entire UI. When React boots up on the client, it starts building this fiber tree from top to bottom, creating a fiber object for every single element in your UI.

Each fiber object has a property called `stateNode`, which holds a direct reference to the actual DOM node.
```tsx
...
During hydration, React walks the fiber tree top to bottom, and instead of creating new DOM nodes, it looks at the existing DOM that came from the server and asks: is there already a node here for this?

If yes, it takes it. It stores that server-rendered DOM node in `stateNode` and attaches all the event listeners. But if the node doesn't match what React expected, say the server rendered a `<div>` but React expected a `<button>`, React has no idea what to do. That's your hydration error.
